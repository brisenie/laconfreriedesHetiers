...
const TABS = [
  ...
  { name: 'passeport', label: 'PASSEPORT', icon: 'passport' as const },
];

function CustomTabBar({ state, navigation }: any) {
  const insets = useSafeAreaInsets();
  return (
    <View style={[styles.tabBar, { paddingBottom: Math.max(insets.bottom, spacing.sm) }]} testID="bottom-tab-bar">
        ...
        return (
          ...
          >
            <MaterialCommunityIcons
              name={tab.icon}
              size={24}
              color={isFocused ? colors.brandPrimary : colors.onSurfaceSecondary}
            />
